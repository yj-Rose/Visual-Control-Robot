# 修改版：用多个指根（MCP）做位置控制 + 深度中值 + EMA 平滑 + 跳变限幅
import time
import cv2
import numpy as np
...
last_cmd_joints = None

def normalize(v: np.ndarray) -> np.ndarray:
    n = np.linalg.norm(v)
    return v / n if n > 1e-9 else v

def wrap_angle_diff_deg(d: np.ndarray) -> np.ndarray:
    """把角度差 wrap 到 [-180, 180) 区间，避免 179 -> -179 这种数值跳变被误判为大步长。"""
    return (d + 180.0) % 360.0 - 180.0

def limit_joint_step_deg(target_joints: np.ndarray,
                         last_cmd_joints: np.ndarray,
                         max_step_deg: np.ndarray) -> np.ndarray:
    """关节空间“每帧最大步长”限幅（单位：度）。"""
    dj = wrap_angle_diff_deg(target_joints - last_cmd_joints)
    dj = np.clip(dj, -max_step_deg, max_step_deg)
    return last_cmd_joints + dj
...
last_cmd_joints = np.array(arm.rm_get_joint_degree()[1], dtype=np.float64)
...
# 关节空间限幅：每帧每轴最大变化（单位：度/帧）
# 经验值：前 3 轴相对保守、腕部可稍大；如果觉得跟手太慢就逐步加大
MAX_DJOINT_DEG = np.array([6.0 , 6.0, 6.0, 6.0, 6.0, 6.0], dtype=np.float64)
...
try:
    while True:
        ...
        if res.multi_hand_landmarks:
            ...
            # IK seed：优先用“上一次下发”的关节角，能显著减少跳解
            q_seed = last_cmd_joints.tolist() if last_cmd_joints is not None else q_current
            params = rm_inverse_kinematics_params_t(q_seed, target_pose, 1)
            ret_ik, joints = algo.rm_algo_inverse_kinematics(params)
            if ret_ik == 0:
                # ---- 关节空间限幅（度/帧）----
                j = np.array(joints, dtype=np.float64)
                if last_cmd_joints is None:
                    last_cmd_joints = np.array(q_current, dtype=np.float64)

                # 先对 IK 结果做限幅
                j_limited = limit_joint_step_deg(j, last_cmd_joints, MAX_DJOINT_DEG)

                # 队列平均平滑（保留）；平均后再限幅一次，避免平均造成单帧回跳
                joint_queue.append(j_limited)
                avg = np.mean(np.vstack(joint_queue), axis=0)
                #avg_limited = limit_joint_step_deg(avg, last_cmd_joints, MAX_DJOINT_DEG)

                arm.rm_movej_follow(avg.tolist())
                last_cmd_joints = avg.copy()
            else:
                if ret_ik == 1:
